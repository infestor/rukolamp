...
inline void next_mode() {
	...
	if(fast_presses[0] == 3) {  // triple-tap from a solid mode
		// go to next mode
		actual_mode++;
	...
	//handle blinkies, bike mode etc..
}

inline void count_num_levels_for_mode(uint8_t target_group) {
...
inline void set_output_pwm(uint8_t pwm1) {
	PWM_LVL = pwm1;
}

void set_level(uint8_t level_id) {
	uint8_t pwm_value = pgm_read_byte(pwm_ramp_values  + level_id - 1);
	if (pwm_value < 15) { TCCR0A = PHASE; }
	set_output_pwm(pwm_value);
}
...
int main(void)
{
	...
	count_num_levels_for_mode(actual_mode); // Enable the current mode group
